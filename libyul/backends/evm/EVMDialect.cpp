...
map<YulString, BuiltinFunctionForEVM> createBuiltins(langutil::EVMVersion _evmVersion, bool _objectAccess)
{
	...
	if (_objectAccess)
	{
		builtins.emplace(createFunction("datasize", 1, 1, true, true, [](
		...
		builtins.emplace(createFunction("dataoffset", 1, 1, true, true, [](
			FunctionCall const& _call,
			AbstractAssembly& _assembly,
			BuiltinContext& _context,
			std::function<void()>
		...
		builtins.emplace(createFunction("datacopy", 3, 0, false, false, [](
			FunctionCall const&,
			AbstractAssembly& _assembly,
			BuiltinContext&,
			std::function<void()> _visitArguments
...
{
	static map<langutil::EVMVersion, shared_ptr<EVMDialect const>> dialects;
	if (!dialects[_version])
		dialects[_version] = make_shared<EVMDialect>(AsmFlavour::Loose, false, _version);
...
EVMDialect const& EVMDialect::strictAssemblyForEVM(langutil::EVMVersion _version)
{
	static map<langutil::EVMVersion, shared_ptr<EVMDialect const>> dialects;
	if (!dialects[_version])
		dialects[_version] = make_shared<EVMDialect>(AsmFlavour::Strict, false, _version);
	return *dialects[_version];
}
...
EVMDialect const& EVMDialect::strictAssemblyForEVMObjects(langutil::EVMVersion _version)
{
	static map<langutil::EVMVersion, shared_ptr<EVMDialect const>> dialects;
	if (!dialects[_version])
		dialects[_version] = make_shared<EVMDialect>(AsmFlavour::Strict, true, _version);
	return *dialects[_version];
}
...
EVMDialect const& EVMDialect::yulForEVM(langutil::EVMVersion _version)
{
	static map<langutil::EVMVersion, shared_ptr<EVMDialect const>> dialects;
	if (!dialects[_version])
		dialects[_version] = make_shared<EVMDialect>(AsmFlavour::Yul, false, _version);
	return *dialects[_version];
}
